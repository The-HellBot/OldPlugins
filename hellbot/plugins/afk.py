...
async def set_not_afk(event):
    ...
    if "#" not in current_message and gvarstat(f"AFK_{ForGo10God}") == "YES":
...
@hell_handler(func=lambda e: bool(e.mentioned or e.is_private))
async def on_afk(event):
    ...
    if "afk" in current_message_text:
        return False
    if gvarstat(f"AFK_{ForGo10God}") == "YES" and not (await event.get_sender()).bot:
        msg = None
        if reason == "":
...
async def _(event):
    ...
    if gvarstat(f"AFK_{ForGo10God}") != "YES":
